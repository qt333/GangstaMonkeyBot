...
@retry(
    ...
    before_sleep=lambda retry_state, **kwargs: logger.info(f"{retry_state.args[0]} | Retrying claim_reward... {retry_state.outcome.exception()}"),
    reraise=True
    )
def claim_reward(session_name: str, method: str) -> dict:
    ...
    if method == 'POST':
        ...
        response = requests.get(url, headers=headers, json=payload, proxies=proxy, timeout=30)
